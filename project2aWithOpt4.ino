...
void setup()
{
    ...
    GetFileInfo(); // MUST be before menu
    // FOR OPTION 4
}
//
...
{
    oLCD.LCDInitialize(LANDSCAPE, false);
    oLCD.print("CONNECTING", 10, 20);
    oLCD.print("TO WIFI", 10, 40);
    oLCD.print(".....", 10, 60);

    HTTPClient m_http;

    if (objHTTP.connect(30))
    {
        if (objHTTP.getURL(m_http,
                           "https://intranet.mne.ksu.edu/static/ME400/images.php", 3000))
        {

            GetBaseFiles(m_http, picFiles, audioFiles, 16);
...
void MysteryPic(String filename)
{
    ...
    HTTPClient m_http;
    unsigned char img[2560];

    if (!objHTTP.connect(30))
        return;

    String url = "https://intranet.mne.ksu.edu/static/ME400/" + filename;

    if (objHTTP.getURL(m_http, url, 3000))
    {

        String data = m_http.getString();

        int idx = data.indexOf("128");
        idx = data.indexOf('\n', idx);

        if (cleanData.length() < 2560)
            return;

        for (int i = 0; i < 2560; i++)
        {

            unsigned char c = (unsigned char)cleanData[i];
        ...
        oLCD.drawXBitmap(0, 0, img, 160, 128, RCB_WHITE);
...
void MysteryAudio(String filename)
{
    ...
    sigmaDeltaSetup(audioPin, audioChannel, 2000000);
    sigmaDeltaWrite(audioChannel, 0);
    ...
    if (objHTTP.getURL(m_http, url, 3000))
    {
        ...
        while (m_http.connected() && (len > 0 || len == -1))
        {
            ...
            if (size)
            {
                ...
                // REQUIRED LOOP (MOST IMPORTANT PART)
                for (int i = 0; i < c; i++)
                {

                    while (!circle_put(buff[i]))
                    {
                        delay(10);
                    }
                }
    ...
    void PlayAudio_loop(void *parameter)
    {
        while (true)
        {
            if (circle_available())
            {
                uint8_t val = circle_get();
                sigmaDeltaWrite(audioChannel, val);
            }
        }
    }
}
